...
class DNN(nn.Module):
    ...
    def __init__(self, inputs_dim, hidden_units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
        ...
        hidden_units = [inputs_dim] + list(hidden_units)  # [16] + [64, 32] --> [16, 64, 32]

        self.linears = nn.ModuleList(
            [nn.Linear(hidden_units[i], hidden_units[i + 1]) for i in range(len(hidden_units) - 1)])  # [nn.Linear(16, 64), nn.Linear(64, 32)]
        ...
        for name, tensor in self.linears.named_parameters():  # 访问所有参数（以迭代器的形式返回）
            if 'weight' in name:
                nn.init.normal_(tensor, mean=0, std=init_std)  # 初始化正太分布
