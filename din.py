...
from .basemodel import BaseModel  # '.'代表当前目录
from ..inputs import *  # '..'代表上一层目录
from ..layers import *
...
class DIN(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, history_feature_list, dnn_use_bn=False,
        ...
        # seed：随机种子 | task："binary"表示二进制对数损失或"regression"用于回归损失 | device: "cpu" or "cuda:0" | gpus：多个gpu的int或torch.device列表
